...
(async () => {
    ...
    const conexaoBot = {
        ...
        async reenviarComFormatoAlternativo(numeroOriginal, texto, imagem = null) {
            console.log(`[conexaoZap.js] reenviarComFormatoAlternativo: Tentando reenvio para ${numeroOriginal}`);
            
            // Limpa o número
            const numeroLimpo = numeroOriginal.replace(/\D/g, '');
            
            // Se não tem 11 dígitos, não podemos fazer a conversão
            if (numeroLimpo.length !== 11) {
                return { erro: 'Número deve ter 11 dígitos para conversão alternativa' };
            }
            
            const DDD = numeroLimpo.substr(0, 2);
            const numeroSemDDD = numeroLimpo.substr(2);
            
            // Se não tem 9 dígitos após DDD, não podemos remover
            if (numeroSemDDD.length !== 9) {
                return { erro: 'Número deve ter 9 dígitos após DDD para conversão alternativa' };
            }
            
            // Gera formato alternativo removendo primeiro dígito após DDD
            const numeroAlternativo = DDD + numeroSemDDD.substr(1);
            console.log(`[conexaoZap.js] reenviarComFormatoAlternativo: ${numeroOriginal} → ${numeroAlternativo}`);
