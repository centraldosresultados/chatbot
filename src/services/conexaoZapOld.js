...
(async () => {
  const conexaoBot = {
    ...
    async enviarMensagem(destinatario, texto, imagem) {
      ...
      if (imagemEnviar && temTexto) textoEnviar = { caption: texto };
      else if (!temImagem && temTexto) textoEnviar = texto;
      ...
      return await new Promise((resolve) => {
        ...
        if (temImagem && temTexto) {
          this.clientBot
            .sendMessage(numero, imagemEnviar, textoEnviar)
          ...
          this.clientBot
            .sendMessage(numero, textoEnviar)
            .then((retornoMensagem) => {
              resolve(this.retornoMensagem(retornoMensagem._data.id.id));
            })
            .catch((error) => {
          ...
          this.clientBot
            .sendMessage(numero, imagemEnviar)
            .then((retornoMensagem) => {
              resolve(this.retornoMensagem(retornoMensagem._data.id.id));
            })
            .catch((error) => {
    ...
    async recebeMensagem(mensagem) {
      ...
      const eResposta =
        ...
        mensagem._data.quotedStanzaID != undefined;
      ...
      if (eResposta && idUsuario) {
        ...
        if (vinc != undefined) {
          /*const resposta = */ await vinculacaoes.responderSolicitacao(
            vinc,
            texto,
            origem,
          );
